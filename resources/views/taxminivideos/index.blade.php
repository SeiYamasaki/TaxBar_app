                    ...
                    <div class="video-card">
                        <a href="{{ route('taxminivideos.show', $video->id) }}"
                            ...
                            <div class="flex items-center mt-2 bg-gray-50 p-2 rounded-lg">
                                <div class="mr-3">
                                    @if ($video->user && $video->expert_photo_url)
                                        <img src="{{ asset('storage/' . $video->expert_photo_url) }}" alt="専門家の写真"
                                            class="user-icon rounded-full w-12 h-12 object-cover border-2 border-gray-200">
                                    @else
                                ...
                                <div>
                                    <p class="text-color-primary font-medium text-black">
                                        投稿者：{{ $video->user->name ?? $video->name }}</p>
                                </div>
                            </div>
                            <div class="mt-2">
                                <a href="{{ route('taxminivideos.show', $video->id) }}"
                                    class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-all duration-300 inline-block">投稿する</a>
                            </div>
                        </a>
                    </div>
                @endforeach
